...
const Attendance = ({handleAttendance}) => {
  ...
  const setAttendance = (regno, status) =>{
    const ChangeAttendance = Student.map((lists)=>lists.regno===regno ? {...lists,status}:lists)
    setStudent(ChangeAttendance)
    handleAttendance(ChangeAttendance)
  }
  return (
    ...
    <table border="1">
      ...
      <tbody>
      {Student.map((items) => (<tr key={items.sno}>
            <td>{items.sno}</td>
            <td>{items.regno}</td>
            <td>{items.name}</td>
            <td><button className="set" onClick={() => setAttendance(items.regno, "PRESENT")}>PRESENT</button></td>
            <td><button className="set1" onClick={() => setAttendance(items.regno, "ABSENT")}>ABSENT</button> </td>
          </tr>))}
      </tbody>
      </table>
    </div>
